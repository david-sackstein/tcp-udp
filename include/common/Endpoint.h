...
struct Endpoint {
    ...
    explicit Endpoint(std::string addr, uint16_t p) : address(std::move(addr)), port(p) {}
    ...
    static Endpoint any_loop_back() {
        return loop_back(any_port);
    }

    static Endpoint loop_back(uint16_t p) {
        return Endpoint(localhost, p);
    }
