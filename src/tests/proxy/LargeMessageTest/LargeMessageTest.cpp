...
std::vector<uint16_t> LargeMessageTest::setupProxyChain() {
    logger_->log(
        logger::LogLevel::INFO, "Setting up proxy chain: client -> server_proxy -> client_proxy -> echo_server");
...
std::shared_ptr<tcp::ITcpSession> LargeMessageTest::createAndConnectClient(uint16_t target_port) {
    ...
    auto session = client_->connect(Endpoint::loop_back(TCP_CLIENT_PORT), Endpoint::loop_back(target_port));
...
bool LargeMessageTest::verifyMessageIntegrity(const std::string& original, const std::string& received) {
    if (original.length() != received.length()) {
        logger_->log(logger::LogLevel::ERROR, "Message length mismatch: original=%zu, received=%zu", original.length(),
            received.length());
        return false;
    }
...
std::vector<uint16_t> LargeMessageTest::setupTestInfrastructure(bool useProxies) {
    logger_->log(
        logger::LogLevel::INFO, "Setting up test infrastructure (useProxies=%s)", useProxies ? "true" : "false");
...
std::string LargeMessageTest::receiveAndVerifyMessage(std::shared_ptr<tcp::ITcpSession> session,
...
std::string LargeMessageTest::handleMessageFragmentation(std::shared_ptr<tcp::ITcpSession> session,
    size_t expectedSize) {
    ...
    while (totalReceived < expectedSize) {
        ...
        Buffer read_buffer{receive_buffer.view().data + totalReceived,
        ...
        if (read_result.code == IOResultCode::Timeout) {
            logger_->log(
                logger::LogLevel::ERROR, "Read timeout, received %zu/%zu bytes so far", totalReceived, expectedSize);
            continue;
        }
        ...
        logger_->log(logger::LogLevel::INFO, "Read %zu bytes, total received: %zu/%zu", read_result.count,
