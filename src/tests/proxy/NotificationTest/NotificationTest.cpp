...
std::tuple<std::string, std::string, std::string> MultiClientNotificationTest::readAllMessages(
    ...
    for (int attempt = 0; attempt < 60; ++attempt) {
        ...
        if (!session1Messages.empty()) {
            client1_messages.insert(client1_messages.end(), session1Messages.begin(), session1Messages.end());
            received_any = true;
        }

        auto session2Messages = readMessagesFromSession(session2);
        if (!session2Messages.empty()) {
            client2_messages.insert(client2_messages.end(), session2Messages.begin(), session2Messages.end());
            received_any = true;
        }

        auto session3Messages = readMessagesFromSession(session3);
        if (!session3Messages.empty()) {
            client3_messages.insert(client3_messages.end(), session3Messages.begin(), session3Messages.end());
            received_any = true;
        }
    ...
    // Combine messages
    std::string client1_combined = combineMessages(client1_messages);
    std::string client2_combined = combineMessages(client2_messages);
    std::string client3_combined = combineMessages(client3_messages);
