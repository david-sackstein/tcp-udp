...
UdpForwarder::UdpForwarder(logger::ILogger& logger, Endpoint udp_proxy, udp::IUdpClient& udp_client)
    : logger_(logger), udp_proxy_(std::move(udp_proxy)), udp_client_(udp_client) {}

void UdpForwarder::forwardTcpToUdp(tcp::ITcpSession& tcp_session,
...
void UdpForwarder::forwardToUdpProxy(const IOResult& tcp_data, const std::string& client_key) {
    ...
    if (!udp_client_.send_to(udp_proxy_, data_to_forward)) {
        ...
        logger_.log(
            logger::LogLevel::INFO, "UdpForwarder: Forwarded TCP data from %s to UDP proxy", client_key.c_str());
    }
}