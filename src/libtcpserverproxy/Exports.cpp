...
namespace server_proxy {
    ...
    EXPORTED std::unique_ptr<IBlockingServer> create_tcp_server_proxy(
        logger::ILogger& logger, const Endpoint& local_endpoint, const Endpoint &udp_proxy) {
        return std::make_unique<TcpServerProxy>(logger, local_endpoint, udp_proxy);
    }

    EXPORTED std::unique_ptr<IBackgroundServer>
    start_tcp_server_proxy(logger::ILogger& logger, const Endpoint& local_endpoint, const Endpoint &udp_proxy) {
        return startServer(
                [&] { return create_tcp_server_proxy(logger, local_endpoint, udp_proxy); },
