...
ServerProxyUdpHandler::ServerProxyUdpHandler(logger::ILogger& logger)
    : logger_(logger) {}
        ...
        for (auto& [client_key, tcp_session] : tcp_sessions_) {
            if (tcp_session) {
                ...
                if (write_result.code == IOResultCode::Success) {
                    logger_.log(logger::LogLevel::INFO, "ServerProxyUdpHandler: Forwarded UDP response to TCP client %s", client_key.c_str());
                } else {
                    logger_.log(logger::LogLevel::ERROR, "ServerProxyUdpHandler: Failed to forward to TCP client %s: %s", 
...
void ServerProxyUdpHandler::register_tcp_session(const std::string& client_key, std::shared_ptr<tcp::ITcpSession> tcp_session) {
    tcp_sessions_[client_key] = tcp_session;
            logger_.log(logger::LogLevel::INFO, "ServerProxyUdpHandler: Registered TCP session for client %s", client_key.c_str());
}

void ServerProxyUdpHandler::unregister_tcp_session(const std::string& client_key) {
    tcp_sessions_.erase(client_key);
            logger_.log(logger::LogLevel::INFO, "ServerProxyUdpHandler: Unregistered TCP session for client %s", client_key.c_str());
} 