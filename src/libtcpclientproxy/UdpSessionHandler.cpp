...
UdpSessionHandler::UdpSessionHandler(logger::ILogger& logger)
    : logger_(logger) {}
...
ConstBuffer UdpSessionHandler::readUdpMessage(udp::IUdpSession& client_session, Endpoint& udp_sender) {
    ...
    if (udp_read_result.code == IOResultCode::Error) {
        throw std::runtime_error(format_string(
            "UdpSessionHandler: %s failed to read UDP: %s",
            udp_sender.to_string().c_str(),
            udp_read_result.error_message.c_str()));
    }

    if (udp_read_result.code == IOResultCode::ConnectionClosed) {
        logger_.log(logger::LogLevel::INFO, "UdpSessionHandler: %s read UDP ConnectionClosed", udp_sender.to_string().c_str());
        return {};
    }
...
void UdpSessionHandler::sendResponseToUdp(udp::IUdpSession& client_session, ConstBuffer response, const Endpoint& udp_sender) const {
    ...
    if (udp_write_result.code == IOResultCode::Error) {
        throw std::runtime_error(format_string(
            "UdpSessionHandler: %s failed to send UDP response: %s",
            udp_sender.to_string().c_str(),
            udp_write_result.error_message.c_str()));
    }
    
    if (udp_write_result.code == IOResultCode::ConnectionClosed) {
        logger_.log(logger::LogLevel::INFO, "UdpSessionHandler: %s write UDP ConnectionClosed", udp_sender.to_string().c_str());
        return;
    }
