...
public:
    UdpTcpBinding(std::unique_ptr<tcp::ITcpClient> client,
        const std::shared_ptr<tcp::ITcpSession>& tcp_session,
        udp::IUdpSession& udp_session,
        const Endpoint& udp_sender)
        : client_(std::move(client)), tcp_session_(tcp_session), udp_session_(udp_session), udp_sender_(udp_sender) {}
    ...
    [[nodiscard]] tcp::ITcpSession& get_tcp_session() const {
        return *tcp_session_;
    }
    [[nodiscard]] const Endpoint& get_udp_sender() const {
        return udp_sender_;
    }
