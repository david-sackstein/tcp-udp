...
BindingFactory::BindingFactory(logger::ILogger& logger,
    Endpoint tcp_server_endpoint,
    std::shared_ptr<ACE_Reactor> reactor)
    : logger_(logger), tcp_server_endpoint_(std::move(tcp_server_endpoint)), reactor_(std::move(reactor)) {}

std::unique_ptr<UdpTcpBinding> BindingFactory::createBinding(const std::string& source_key,
