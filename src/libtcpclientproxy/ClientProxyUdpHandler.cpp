...
std::unique_ptr<ITask> ClientProxyUdpHandler::handle_client(udp::IUdpSession& client_session) {
    try {
        Endpoint udp_sender;
        ...
        logger_.log(logger::LogLevel::ERROR, "ClientProxyUdpHandler: Exception: %s", e.what());
        return nullptr;
    }
}
