...
namespace udp {
    ...
    EXPORTED std::unique_ptr<IBlockingServer> create_udp_server(logger::ILogger& logger, const Endpoint& local_endpoint, IUdpClientHandler &handler) {
        return std::make_unique<AceUdpServer>(logger, local_endpoint, handler);
    }

    EXPORTED std::unique_ptr<IBackgroundServer> start_udp_server(
        logger::ILogger& logger, const Endpoint& local_endpoint, IUdpClientHandler &handler) {
        return startServer(
                [&] { return create_udp_server(logger, local_endpoint, handler); },
