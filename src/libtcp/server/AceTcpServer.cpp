...
AceTcpServer::AceTcpServer(logger::ILogger& logger, const Endpoint& local_endpoint, tcp::ITcpClientHandler& handler)
    ...
    if (acceptor_.open(local_endpoint) != 0) {
        ACE_ERROR((LM_ERROR, ACE_TEXT("Failed to open acceptor on %s, errno: %d (%s)\n"),
            local_endpoint.to_string().c_str(), ACE_OS::last_error(), ACE_OS::strerror(ACE_OS::last_error())));
...
AceTcpServer::AceTcpServer(logger::ILogger& logger,
    ...
    logger_.log(logger::LogLevel::INFO, "TcpServer: %s successfully bound with external reactor",
...
void AceTcpServer::start() {
    ...
    if (status != 0) {
        ACE_ERROR(
            (LM_ERROR, ACE_TEXT("run_reactor_event_loop returned: %d, errno: %d\n"), status, ACE_OS::last_error()));
    }
}
